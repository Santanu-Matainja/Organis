			...
			<div class="col-lg-12">
				<div class="card">
					...
					<div class="card-body">
						<div class="row mb-10">
							<div class="col-lg-8">
								<div class="group-button">
									<button id="orderstatus_0" type="button" onClick="onDataViewByStatus(0)" class="btn btn-theme orderstatus active">{{ __('All') }} (@php echo AdminOrderCount(0); @endphp)</button>
									@foreach($order_status_list as $row)
									<button id="orderstatus_{{ $row->id }}" type="button" onClick="onDataViewByStatus({{ $row->id }})" class="btn btn-theme orderstatus">{{ $row->ostatus_name }} (@php echo AdminOrderCount($row->id); @endphp)</button>
									@endforeach
								</div>
						...
						<div id="tp_datalist">
							@include('backend.partials.Admin_orders_table')
						</div>
					</div>
					<!--/Data grid/-->
				</div>
...
@push('scripts')
<!-- css/js -->
<link rel="stylesheet" href="{{asset_path('backend/bootstrap-datetimepicker/bootstrap-fonticon.css')}}">
<link rel="stylesheet" href="{{asset_path('backend/bootstrap-datetimepicker/bootstrap-datetimepicker.css')}}">
<script src="{{asset_path('backend/bootstrap-datetimepicker/bootstrap-datetimepicker.min.js')}}"></script>
...
<script src="{{asset_path('backend/pages/Adminorders.js')}}"></script>
@endpush