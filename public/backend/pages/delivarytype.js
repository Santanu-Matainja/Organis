...
function onPaginationDataLoad(page) {
	$.ajax({
		url:base_url + "/backend/getDelivarytypeTableData?page="+page+"&search="+$("#search").val()+"&status="+$("#view_by_status").val(),
...
function onRefreshData() {
	$.ajax({
		url:base_url + "/backend/getDelivarytypeTableData?search="+$("#search").val()+"&status="+$("#view_by_status").val(),
		success:function(data){
			$('#tp_datalist').html(data);
			onCheckAll();
		}
...
function onDataViewByStatus(status) {
	...
	$.ajax({
		url: base_url + "/backend/getDelivarytypeTableData?status="+$("#view_by_status").val()+"&search="+$("#search").val(),
		success:function(data){
			$('#tp_datalist').html(data);
			onCheckAll();
		}
...
function onConfirmWhenAddEdit() {
    ...
    $.ajax({
		type : 'POST',
		url: base_url + '/backend/saveDelivarytypeData',
...
function onLoadEditData() {
    ...
    $.ajax({
		type : 'POST',
		url: base_url + '/backend/getDelivarytypeById',
		data: 'id='+RecordId,
		success: function (response) {

			var delivarytype_data = response.delivarytype_data;
			...
			if(delivarytype_data.status_id == 1){
			...
			$("#created_at").text(delivarytype_data.created_at);
...
function onConfirmDelete() {
    ...
    $.ajax({
		type : 'POST',
		url: base_url + '/backend/deletedelivarytype',
		data: 'id='+RecordId,
		success: function (response) {
			var msgType = response.msgType;
			var msg = response.msg;
...
function onConfirmBulkAction() {
    ...
    $.ajax({
		type : 'POST',
		url: base_url + '/backend/bulkActiondelivarytype',
...
document.addEventListener('DOMContentLoaded', function() {
    const labelInput = document.getElementById('lable');
